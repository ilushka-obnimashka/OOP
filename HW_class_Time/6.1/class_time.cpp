...
/*int main() {
    ...
    // б) Создать стандартные контейнеры (vector, list и т.д.), содержащие объекты типа Time.
    {
        ...
        /*вызовется size конструкторов.
        деструктор класса будет вызван автоматически т.к. деструкторы автоматически вызываются  для  объектов являющихся элементами контейнеров.#1#
    }
    std:: cout << GREEN << "creating vector<Time>with Copy Constructor:\n" << RESET;
    {
        ...
        т.к. деструкторы автоматически вызываются  для  объектов являющихся элементами контейнеров и временных объектов.#1#
    }
