...
{
    uint64_t offset = 2 * static_cast<uint64_t>(WAVHeader_->sample_rate) * static_cast<uint64_t>(start) + static_cast<
    ...
    if (current_pos_in_file <= offset)
    {
        ...
        remaining_audio_data_size_ = static_cast<uint64_t>(end - start) * static_cast<uint64_t>(WAVHeader_->
            sample_rate);
    }
